...
namespace DUSI_COMMON {
	...
	BOOL ReSizeDlg(const HWND hDlg, const DOUBLE ratio)
	{
		...
		if (!SetWindowPos(hDlg, NULL
			, 0
			, 0
			, static_cast<int>((rcPrevDlg.right - rcPrevDlg.left) * ratio)
			, static_cast<int>((rcPrevDlg.bottom - rcPrevDlg.top) * ratio)
		...
		RECT rect(0
					 , 0
					 , static_cast<LONG>((rcPrevDlg.right - rcPrevDlg.left) *ratio)
					 , static_cast<LONG>((rcPrevDlg.bottom - rcPrevDlg.top) *ratio));
