...
MainDialog()
		...
		if (CtrlType == CTRL_C_EVENT) {
		  ...
		  // プロセスIDを取得する
			...
			string pipename(string("\\\\.\\pipe\\")\
				+ _CRT_STRINGIZE(NAMED_PIPE_PREFIX)\
				+ to_string((int)pid));
			...
			size_t size = strCtrlC.size();
			WriteFile(hPipe, strCtrlC.c_str(), (DWORD)strCtrlC.size(), NULL, NULL);
	...
	)
	{
		...
		if (!ReadFile(pDlg->hIn, pDlg->ParentReadBuf.data(), (DWORD)pDlg->ParentReadBuf.size(), &cbRead, NULL))
		{
			ErrOut(GetLastError());
			CloseThreadpoolWait(Wait);
			return;
	...
	)
	{
		...
		DWORD cbRead(0);
		if (!ReadFile(pDlg->hUpRd, pDlg->ChildReadBuf.data(), (DWORD)pDlg->ChildReadBuf.size(), &cbRead, NULL))
		{
			ErrOut(GetLastError());
			CloseThreadpoolWait(Wait);
			return;
	...
	)
	{
		...
		DWORD Writen(0);
		if (!WriteFile(pDlg->hOut, str.data(), (DWORD)str.size(), &Writen, NULL))
		{
			ErrOut(GetLastError());
		}
	...
	)
	{
		...
		DWORD Writen(0);
		if (!WriteFile(pDlg->hDownWt, strsrc.data(), (DWORD)strsrc.size(), &Writen, NULL))
		{
			ErrOut(GetLastError());
			return;
		}
	...
	[](
	 ...
	 _In_        TP_WAIT_RESULT        WaitResult
		...
		if (!ReadFile(	
			...
			, (DWORD)pDlg->olConReadComp.str.size()
			, NULL
			, &pDlg->olConReadComp)) {
			if (DWORD dw = GetLastError() != ERROR_IO_PENDING) {
	...
	) 
	{
		...
		RECT rcDlgCli;
		if (!GetClientRect(pDlg->hDlgMain, &rcDlgCli))
		{
			EOut;
			return;
		}
		...
		UIDialog* pUI(NULL);
		{
			...
			GetWindowRect(pUI->hDlg, &rcDlgUI);
		}
		if (!::MapWindowRect(
			HWND_DESKTOP
			, pDlg->hDlgMain
	...
	// List1設定
		...
		// List1、RECTを求める。
		RECT rcListWin;
		if (!::GetWindowRect(hList, &rcListWin))
		{
			EOut;
			return;
		}
		...
		// リストビューヘッダー等の設定
		{
			// アラインヘッダー
			{
				...
				GetTextExtentPoint32(hDC, strH0.c_str(), (DWORD)strH0.size(), &sz);
		...
		{
			RECT rcListWinAfter;
			if (!::GetWindowRect(hList, &rcListWinAfter))
			{
				EOut;
				return;
			}
...
// コンソール表示
	...
	if(hwndCon)
	{
		...
		// プロセスIDを名前に加える。
		string pipename(string("\\\\.\\pipe\\")\
			+ _CRT_STRINGIZE(NAMED_PIPE_PREFIX) + to_string((int)pid));
	...
	wstring strParent = Exepath.parent_path().c_str();
...
BOOL UsiDebugStb::MainDialog::ChangeSimpleUIMode()
{
	...
	RECT rcDlgNew;
	GetWindowRect(hDlgMain, &rcDlgNew);
	if (!WritePrivateProfileStruct(
		_T("MAIN_DLG")
		, _T("WINDOW_POS")
		, &rcDlgNew
...
BOOL UsiDebugStb::MainDialog::ChangeNormalUIMode()
{
	...
	RECT rcNormal;
	GetWindowRect(pNormalUIDlg->hDlg, &rcNormal);
	OffsetRect(&rcNormal, -rcNormal.left, -rcNormal.top);
	RECT rcMinimum;
	GetWindowRect(pMinimumUIDlg->hDlg, &rcMinimum);
	OffsetRect(&rcMinimum, -rcMinimum.left, -rcMinimum.top);
	LONG dest = rcNormal.bottom - rcMinimum.bottom;
	RECT rcDlg;
...
//	_D("SetInitialPlacement");
	...
	// リストビューは特別設定
	{
		...
		// アラインヘッダー
		{
			...
			SIZE sz{};
			tstring strH0(_T("1000000"));
			GetTextExtentPoint32(hDC, strH0.c_str(), (int)strH0.size(), &sz);
			SelectFont(hDC, hfontprev);
			DeleteFont(hfont);
...
OnEdtMaxRaw(HWND hwnd, HWND hctrl, UINT code)
{
	...
	switch (code)
	{
		case EN_CHANGE:
		{
			...
			BOOL bFalse(0);
			int iPos = (int)::SendMessage(hudMaxRaw, UDM_GETPOS32, 0, (LPARAM)&bFalse);
			if (bFalse)
			{
				MessageBox(hDlgMain
...
INT_PTR UsiDebugStb::MainDialog::OnWmSizing(HWND hwnd, UINT state, RECT *pRect)
{
	...
	UIDialog* pUI(NULL);
	{
		...
		OffsetRect(&rcDlgUI, -rcDlgUI.left, -rcDlgUI.top);
	}

	// UIダイアログを最小サイズにした場合の、MainDialogの最小RECTを得る。
	RECT rcWin = rcDlgUI;
	if (!AdjustWindowRect(& rcWin, WS_OVERLAPPEDWINDOW, FALSE)) {
		EOut;
		return FALSE;
	}
...
{
	if (wParam != reinterpret_cast<WPARAM>(hDlg)) {
		return FALSE;
	}
	switch (lParam) {
		case WM_LBUTTONDOWN:
...
INT_PTR UsiDebugStb::MainDialog::OnWmUserSetFont(HWND hwndCtl, HFONT hfont)
{
	...
	// メインダイアログのサイズの更新
	{
		const int cx = static_cast<int>((rcConvBaseRatioMain.right - rcConvBaseRatioMain.left) * NewRatio);
		const int cy = static_cast<int>((rcConvBaseRatioMain.bottom - rcConvBaseRatioMain.top) * NewRatio);
		if (!SetWindowPos(
			hwndCtl
			, HWND_TOP
			, 0
	...
	for (const auto& pair : vhrcDlgItemsBasePos)
	{
		if (!::SetWindowPos(
			pair.first
			, HWND_TOPMOST
			, static_cast<int>(pair.second.left * NewRatio)
			, static_cast<int>(pair.second.top * NewRatio)
			, static_cast<int>((pair.second.right - pair.second.left) * NewRatio)
			, static_cast<int>((pair.second.bottom - pair.second.top) * NewRatio)
			,
			SWP_NOOWNERZORDER
			| SWP_SHOWWINDOW
		))
